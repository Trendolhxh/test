...
健康数据（睡眠、心率、血氧等）属于敏感个人信息，需注意：

- LLM 调用需开启 zero data retention（供应商不留存用户数据）
- 注入 prompt 的数据不包含用户身份标识（如姓名、手机号）
- 遵循 App 现有的健康数据授权策略，用户未授权的数据类别不注入
...
| < 3 秒响应可接受 | 超出则需优化模型选择或缓存策略 | 线上 P99 延迟监控 |
