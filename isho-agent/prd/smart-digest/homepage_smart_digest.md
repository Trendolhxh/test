...
2. questions: 3个推荐提问（每个≤10字，不含标点）。一眼能读完，不能有分句或逗号。方向参考：数据下钻、术语解读、改善建议。基于 digest 内容和当前时间灵活生成。

示例：
输入：深睡 21%，HRV +3ms，早上 8:30
输出：{"digest":"深睡 21% 回到线上，HRV 也涨了 3ms——今早的你可以嚣张一点","questions":["深睡为什么会波动","HRV 多少算正常","怎么保持这状态"]}
...
| 摘要上下文 | 推荐提问 |
|:---|:---|
| 深睡重新达标 + HRV 上涨 | ① 深睡为什么会波动 ② 近三天深睡趋势 ③ 怎么保持这状态 |
| 入睡太晚 + 深睡偏低 | ① 晚睡对深睡影响多大 ② 深睡占比多少正常 ③ 今晚怎么改善 |
| 高能时段剩余多 | ① 高能时段怎么算的 ② 今天高能窗口变了吗 ③ 高能时段怎么用好 |
...
| `questions` | 长度为 3 的字符串数组，每项 ≤ 10 字（不含标点），不含分句 | 降级：不展示摘要和快捷提问 |
