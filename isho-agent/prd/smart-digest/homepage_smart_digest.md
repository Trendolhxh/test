...
- **对比逻辑**：下次触发时，逐指标对比当前值与快照，任一指标发生有意义的变化即视为 `has_new_data`
...
diff:
  ...
  changes:
    ...
    - key: "hrv_ms"
      prev: 42 → curr: 45             # Δ +3
    - key: "remaining_h"
      prev: 5.2 → curr: 3.8           # Δ -1.4
```

> **压缩策略**：只传有变化的字段 + delta 值，未变化的字段不出现在 diff 中，节省 token。
...
- [x] 数据更新后自动生成智能摘要（一行）
- [x] 基于摘要和时段生成 3 个快捷提问
- [x] 15 分钟冷却 + 数据 diff 双重触发条件
- [x] 摘要注入 Chatflow 作为对话上下文
...
| 假设 | 影响 | 验证方式 |
|:---|:---|:---|
| ~650 tk 足以生成高质量摘要 | 若不足需扩展 prompt 或换更强模型 | prompt 实测 |
| 用户每日打开 App 3-5 次 | 影响成本估算 | 数据埋点统计 |
| < 2 秒响应可接受 | 超出则需优化模型选择或缓存策略 | 线上 P99 延迟监控 |
| 防重复策略有效 | 若重复率高需追加去重措施 | 人工审核前 100 条 |
