...
- **日期分界线**：凌晨 5:00。5:00 前打开页面仍视为"今晚"，5:00 后推荐的是当晚的入睡时间
- **用户在推荐入睡时间之后打开页面**：推荐入睡时间调整为当前时间之后最近的 15 分钟刻度，起床时间不变
  - 例：推荐入睡时间为 23:00，用户 23:20 打开 → 推荐入睡时间显示为 23:30，起床时间仍为 06:30
  - 高能预测以调整后的时间为基准重新计算
...
Response:
{
  ...
  "optimal_bedtime": "23:30",          // 最优入睡时间（客户端必需，用于推送文案拼接入睡窗口时段）
  "predicted_high_energy": {
    "total_min": 450,                  // 推荐方案下的预测高能时长（分钟）
...
{
  "predicted_high_energy": {
    "total_min": 312,                  // 预测高能时长（分钟）
    "windows": [                       // 高能时段分布
      { "start": "09:30", "end": "11:00" },
      { "start": "14:30", "end": "16:00" }
    ]
  },
  "delta_vs_recommended": -138         // 与推荐方案的高能时长差值（分钟，负数表示减少）
}
```
...
| 就寝拖动点 | 弧线起点，床图标 |
| 起床拖动点 | 弧线终点，太阳图标 |
| 圆心文字 | 当前睡眠时长，格式参考 Figma 设计稿（如 "7h 30min"） |
...
**高能时长展示格式**：`X 小时 Y 分`（由 `total_min` 换算，例：450 分钟 → "7 小时 30 分"，312 分钟 → "5 小时 12 分"，45 分钟 → "45 分钟"）。不足 1 小时时省略小时，只显示"Y 分钟"。
...
> 预估明日高能时长 **5 小时 12 分**
>
> 睡眠比推荐减少 {sleep_delta_display}，明日高能减少 {energy_delta_display}。

**调整态 — 高能增加**（调整后高能高于推荐）：

> 预估明日高能时长 **7 小时 45 分**
>
> 睡眠比推荐增加 {sleep_delta_display}，明日高能增加 {energy_delta_display}。

**调整态 — 高能无变化**（调整后高能与推荐持平）：

> 预估明日高能时长 **7 小时 30 分**
>
> 睡眠比推荐减少/增加 {sleep_delta_display}，明日高能无明显变化。

> - `{sleep_delta_display}`：当前睡眠时长与推荐睡眠时长的差值，客户端本地计算（选中时段时长 - 推荐时段时长）
> - `{energy_delta_display}`：来自 predict 接口的 `delta_vs_recommended`（取绝对值后换算）
> - 格式同高能时长：≥ 60 分钟用"X 小时 Y 分"，< 60 分钟用"Y 分钟"

### 3.3 提醒设置

| 提醒类型 | 默认状态 | 触发时机 | 实现方式 |
|:---|:---|:---|:---|
| 入睡提醒 | **关闭** | 用户实际设置的入睡时间 | 客户端本地推送通知 (UNUserNotificationCenter) |
| 起床闹钟 | **关闭** | 用户实际设置的起床时间 | iOS 系统闹钟 API |

**入睡提醒推送文案**（3 套模板，每次随机选 1 条）：

用户**未调整过时间**（当前时间 = 推荐时间）：
1. "今晚 {recommended_bedtime}–{optimal_bedtime} 是你的最佳入睡窗口，准备休息吧"
2. "你的最佳入睡窗口 {recommended_bedtime}–{optimal_bedtime} 已经到了"
3. "最佳入睡窗口 {recommended_bedtime}–{optimal_bedtime}，早点上床明天更有精力"

用户**调整过时间**（当前时间 ≠ 推荐时间）：
1. "你设定的入睡时间 {selected_bedtime} 到了，准备休息吧"
2. "已经 {selected_bedtime} 了，该准备入睡了"
3. "到 {selected_bedtime} 了，早点上床明天更有精力"

- 推送触发时间 = 用户实际设置的入睡时间（未调整时即为推荐入睡时间）
- 重复策略：每日重复。每次用户打开睡眠窗口页面时，自动更新为最新的推荐时间或用户调整后的时间
- 判断是否调整过：客户端对比 `selected_bedtime == recommended_bedtime && selected_wake_time == regular_wake_time`

**权限处理**：
- 首次开启入睡提醒：检查通知权限，未授权则弹出系统授权弹窗，拒绝后开关自动回到关闭状态
...
- **开启入睡提醒**：立即注册本地推送通知，触发时间为用户当前设置的入睡时间，每日重复
- **关闭入睡提醒**：立即取消已注册的推送通知
- **开启起床闹钟**：调用系统闹钟 API 创建闹钟
- **关闭起床闹钟**：取消已创建的闹钟
- **时间调整联动**：用户拖动调整时间后，若提醒/闹钟已开启，自动更新触发时间和推送文案
- **页面刷新联动**：每次打开睡眠窗口页面，若提醒已开启，自动更新为最新推荐时间（或用户上次调整的时间）
- **状态持久化**：提醒开关状态 + 用户上次调整的时间，本地持久化，下次进入页面恢复
...
| 7 | 用户选择极端时间（睡眠 < 4h） | 允许选择，文案按调整态正常显示差值（如"睡眠比推荐减少 3 小时 30 分，明日高能减少 5 小时 18 分"） |
| 8 | 用户拖回推荐位置 | 恢复为推荐态文案 |
| 9 | 通知权限未授权 | 开启提醒时弹出系统授权弹窗，拒绝后开关自动回到关闭状态 |
| 10 | 网络断开 | 圆盘拖动本地交互正常，松手后提示"网络不可用，无法预测" |
| 11 | 用户在推荐入睡时间之后打开 | 推荐入睡时间调整为当前时间后最近的 15 分钟刻度，起床时间不变 |
