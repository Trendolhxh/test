...
**高能时长展示格式**：`X 小时 Y 分`（由 `total_min` 换算，例：450 分钟 → "7 小时 30 分"，312 分钟 → "5 小时 12 分"，60 分钟 → "1 小时 0 分"）
...
> 预估明日高能时长 **5 小时 12 分**
>
> 比推荐时间晚 {delta_min} 分钟，明日高能减少 {delta_display}。

**调整态 — 早睡**（用户将就寝时间拖早于推荐）：

> 预估明日高能时长 **7 小时 45 分**
>
> 比推荐时间早 {delta_min} 分钟，明日高能增加 {delta_display}。
...
> 其中 `{delta_display}` 同样使用"X 小时 Y 分"格式（例："2 小时 18 分"）。若不足 1 小时则显示"Y 分钟"（例："45 分钟"）。

### 3.4 提醒设置

| 提醒类型 | 默认状态 | 触发时机 | 实现方式 |
|:---|:---|:---|:---|
| 入睡提醒 | **开启** | 入睡窗口最早时间（即推荐入睡时间） | 客户端本地推送通知 (UNUserNotificationCenter) |
| 起床闹钟 | **关闭** | 用户选择的起床时间 | iOS 系统闹钟 API |

**入睡提醒推送文案**（3 套模板，每次随机选 1 条发送）：
1. "今晚 {recommended_bedtime}–{optimal_bedtime} 是你的入睡窗口，准备休息吧"
2. "你的入睡窗口 {recommended_bedtime}–{optimal_bedtime} 已经到了"
3. "最佳入睡窗口 {recommended_bedtime}–{optimal_bedtime}，早点上床明天更有精力"

- `{recommended_bedtime}`：推荐入睡时间（窗口最早时间），例 "23:00"
- `{optimal_bedtime}`：最优入睡时间（窗口最晚时间），例 "23:30"
- 推送触发时间 = `{recommended_bedtime}`（入睡窗口最早时间）
- 示例："今晚 23:00–23:30 是你的入睡窗口，准备休息吧"
...
- **开启入睡提醒**：立即注册本地推送通知（在入睡窗口最早时间触发，即推荐入睡时间）
- **关闭入睡提醒**：立即取消已注册的推送通知
...
| 7 | 用户选择极端时间（睡眠 < 4h） | 允许选择，文案按调整态正常显示差值（如"比推荐时间晚 180 分钟，明日高能减少 5 小时 18 分"） |
| 8 | 用户拖回推荐位置 | 恢复为推荐态文案 |
