...
┌─────────────────────┐           ┌──────────────────────┐
│ 对话中捕获的碎片事实  │           │ 用户画像.md           │
│ · 用户说的每句关键信息 │──→ 子agent │  这个人是谁、睡眠全貌、│
│ · 设备数据快照        │   定期提炼  │  做得好的和待改善的    │
│ · 反馈卡片结果        │           ├──────────────────────┤
│ · ...               │──→ 子agent │ 干预策略.md           │
└─────────────────────┘   定期提炼  │  当前在做什么、试过什么、│
                                   │  什么能用什么别提      │
...
**职责：**
1. 更新用户画像.md — 补充生活细节、刷新作息模式、平衡描述睡眠状况（好的和待改善的）、更新趋势
2. 更新干预策略.md — 记录干预结果、调整当前策略、维护红线

**原则：**
- md 是给主 agent 看的，写成"一个了解用户的睡眠顾问会怎么给同事做交接"的语气
- 两份 md 合计控制在 ~800 token 以内
- 干预历史只保留最近 10 条，更早的提炼为规律后删除
- 描述睡眠状况时必须同时覆盖"做得好的"和"待改善的"，不能只挑问题
