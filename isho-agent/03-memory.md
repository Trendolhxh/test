...
- **原理层**（`principles`）：基于 `睡眠第一性原理.md` 的三件事（什么时候睡 / 睡眠质量 / 白天补充），分析这个用户的精力问题主要出在哪一步。所有认知引导和行动建议都锚定在这里。
- **认知+行动层**（`cognition` + `action`）：分别管理"用户理解了什么"和"用户在做什么"。认知层有学习路径和引导触发器；行动层合并了原 `active` + `preferences`，每个干预说清楚在解决什么问题。
