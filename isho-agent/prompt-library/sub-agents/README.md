...
## 通用规则（所有子 agent MUST 遵守）

### 隔离原则
- 子 agent 只能看到传入的数据，NEVER 访问主对话上下文
- 子 agent 之间 NEVER 直接通信——通过 orchestrator 中转
- 子 agent 不持有状态——每次调用都是无状态的

### 输出规范
- 子 agent 的输出 NEVER 直接展示给用户
- 输出格式 MUST 严格遵循各自模板定义的结构
- 输出中 NEVER 包含对用户的称呼或对话语气——子 agent 不是在和用户说话

### 错误处理
- 子 agent 执行失败时，orchestrator 应静默降级，NEVER 向用户暴露子 agent 的存在
