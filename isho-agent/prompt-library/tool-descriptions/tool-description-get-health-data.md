...
始终先查 7d 看趋势。发现异常再扩展到 14d 或 30d 确认是否持续性问题。一次请求多个相关指标做交叉分析，禁止分多次请求能在一次完成的查询。

## 返回数据为空时

某些指标返回空（用户未佩戴设备、指标不可用）时，用文字告知"这段时间的XX数据暂时没有"，禁止编造数据，禁止忽略空值继续分析。可基于其他有效指标给出有限结论。
