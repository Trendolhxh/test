...
```
┌─────────────────────────────────────────────────────────────┐
│ ① 身份层（01-system-prompt，固定，~250 tk）                   │
...
> **对标 Claude Code**：Claude Code 的 system prompt 合计 ~3,200 tk。精力管家的固定部分（① + ③）只有 ~400 tk，加上动态部分（② + ④）合计 ~580 tk——更轻量，因为详细上下文按需拉取而非预注入。
