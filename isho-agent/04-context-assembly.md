...
**固定部分在前，动态部分在后**——最大化 prompt caching 收益：

1. **① 身份层放最前**——agent 首先知道"我是谁、核心原则是什么"
2. **② 样式指令紧随其后**——和身份层一起构成固定前缀（~350 tk），所有用户、所有会话可共享 prompt cache
3. **③ 用户速览在动态区开头**——cache 在此断裂（每用户不同），但同一用户的同一会话内保持不变
4. **④ 场景指令放最后**——离对话历史最近，和当前上下文关联最强，模型更容易将其与用户消息联系起来

> **Prompt Caching**：Anthropic API 会缓存 system prompt 的相同前缀。① + ② 合计 ~350 tk 是所有用户共享的固定前缀，可以跨请求缓存。③ 开始因用户不同而分叉，但同一用户的同一会话内 ①②③ 都不变（速览由子 agent 在对话结束后异步更新），进一步提升缓存命中率。
...
│                                                  ┌────────┐ │
│ 你是"精力管家"，一位专业的睡眠与精力管理顾问。      │ prompt │ │
│                                                  │ cache  │ │
│ ## 核心原则                                       │ 命中区 │ │
│ 1. 所有建议的最终目标都是：提升睡眠质量和第二天精力  │~350 tk│ │
│ 2. 从用户的真实生活出发，不从数据出发...            │所有用户│ │
│ 3. 持续了解用户...                                │ 共享   │ │
│ 4. 行动建议必须可执行、可反馈...                    │        │ │
│                                                  │        │ │
...
> **对标 Claude Code**：Claude Code 的 system prompt 合计 ~3,200 tk，固定部分在前。精力管家的固定前缀（① + ②）只有 ~350 tk，可跨所有用户缓存。加上动态部分（③ + ④）合计 ~530 tk——更轻量。详细的用户信息按需通过工具加载，不占 system prompt 空间。
...
| 区块 | 预算 | 性质 | 说明 |
|------|------|------|------|
| ① 身份层 | ~200 tk | 固定 | 不随用户变化，可缓存 |
| ② 样式指令 | ~150 tk | 固定 | 不随用户变化，可缓存（①② 合计 ~350 tk 为缓存前缀） |
| ③ 用户速览 | ~80 tk | 动态 | 一行式摘要，从 [summary] section 提取，同一会话内不变 |
